...
int main(int argc, char *argv[])
{
   ...
   // To build H(div) (form 2 in 3D), it is needed to obtain all forms and spaces with larger
   // indices. To use the so called "Hiptmair smoothers", a one form lower is needed
   // (H(curl), form 1, in 3D). To use AMS and ADS, all forms and spaces to H1 (0 form) are needed.
   ...
   const SharingMap& hdiv_dofTrueDof = sequence[0]->GetDofHandler(jform)->GetDofTrueDof();
   ...
   shared_ptr<HypreParMatrix> A;
   {
      ...
      // D1 is the divergence operator from H(div) vector fields to L2 scalar fields.
      // Thus, D1^T * M2 * D1 represents the form (alpha div u, div v) on H(div) vector fields.
      ...
      // spA = D1^T * M2 * D1 + M1 represents the form (alpha div u, div v) + (beta u, v)
      // on H(div) vector fields. This is local, i.e. on all known dofs for the process.
