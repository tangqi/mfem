...
void SysOperator::NonlinearEquationRes(GridFunction &psi, Vector *currents, double &alpha) {
  ...
  if (debug_output) { f_.Save("gf/f.gf"); }
  ...
  // uses the BFS-computed plasma_current.
  {
    ...
    cout << "[I_p comparison] BFS whole-element : " << plasma_current / get_mu() << endl;
    ...
    if (cc_ok) {
      ...
      cout << "[I_p comparison] cut-cell moment   : " << Ip_cutcell / get_mu() << endl;
