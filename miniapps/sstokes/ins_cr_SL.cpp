...
public:
   INSOperator(FiniteElementSpace &vel_fes, FiniteElementSpace &pres_fes, Mesh &mesh, double dim, double visc, 
...
int main(int argc, char *argv[])
{
   ...
   INSOperator oper(*vel_fes, *pres_fes, *mesh, dim, visc, ess_bdr, block_offsets);
   socketstream sout;
   if (visualization)
   {
...
INSOperator::INSOperator(FiniteElementSpace &vel_fes, 
                         FiniteElementSpace &pres_fes, Mesh &mesh, double dim, double visc, 
                         Array<int> &ess_bdr_, 
...
///////////////////////////////////////////SL/////////////////////////////////////////
  ...
  // mform.SpMat().Print();
   ...
   //FIXME!!!
   //Find the matrix B=PM^{-1}; How to get inv(M)?  
   //for each column i of P, we multiply diag(i) of M; then we get P=B; How to implement?
